<template>
  ...
  >
    <div class="chat-container">
      ...
      <!-- 工具调用状态提示 -->
      <div v-if="currentToolAction" class="tool-status">
        <el-tag type="info" size="small">⚙️ {{ currentToolAction }}</el-tag>
      </div>
...
const initMessages = () => {
  messages.value = [
    {
      role: 'assistant',
      content: '你好！我是 Neuro 助手，可以帮你查询训练进度、推荐参数、解释模型原理。试试下面的快捷提问吧！'
    }
  ]
}
...
const send = async () => {
  ...
  // ✅ 新增：设置初始工具状态
  currentToolAction.value = '正在分析问题...'

  try {
    // 发起请求，获取 request_id
    const res = await api.post('/train/agent/chat', {query})

    if (res.data.status === 'processing' && res.data.message) {
      // 被并发控制拦截，直接显示提示
      currentToolAction.value = ''  // ✅ 清除状态
      messages.value[msgIndex].content = res.data.message
      messages.value[msgIndex].isTyping = false
      isTyping.value = false
      saveHistory()
      return
    ...
    // ✅ 更新状态
    currentToolAction.value = '正在查询数据...'
    ...
    const poll = async () => {
      try {
        const resultRes = await api.get(`/train/agent/chat/result/${requestId}`)
        if (resultRes.data.status === 'completed') {
          currentToolAction.value = ''  // ✅ 清除状态
          const answer = resultRes.data.answer
          typewriterEffect(answer, msgIndex)
        } else if (resultRes.data.status === 'failed') {
          currentToolAction.value = ''  // ✅ 清除状态
          messages.value[msgIndex].content = '抱歉，处理失败，请重试。'
          messages.value[msgIndex].isTyping = false
          isTyping.value = false
          saveHistory()
        } else {
          ...
          if (attempts < maxAttempts) {
            ...
            currentToolAction.value = ''  // ✅ 清除状态
            messages.value[msgIndex].content = '请求超时，请稍后再试。'
            messages.value[msgIndex].isTyping = false
            isTyping.value = false
            saveHistory()
          }
        ...
        currentToolAction.value = ''  // ✅ 清除状态
        messages.value[msgIndex].content = '获取结果失败，请重试。'
        messages.value[msgIndex].isTyping = false
        isTyping.value = false
        saveHistory()
      }
    ...
    currentToolAction.value = ''  // ✅ 清除状态
    messages.value[msgIndex].content = e.response?.data?.detail || '请求失败，请重试。'
    messages.value[msgIndex].isTyping = false
    isTyping.value = false
    saveHistory()
  }
